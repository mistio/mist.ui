  ...
  _template: html`
        ...
        <iron-collapse opened="[[expanded]]">
            <template is="dom-repeat" items="[[ruleCategories]]" as="cat">
                <paper-material elevation="0" id\$="[[cat.id]]">
                    ...
                    <template is="dom-repeat" items="[[cat.rules]]">
                        <rule-item hidden\$="[[item.editing]]" editing="{{item.editing}}" rule="[[item]]" available-metrics="[[availableMetrics]]" model="[[model]]" users="[[users]]" teams="[[teams]]" open-incident="[[_hasOpenIncident(item,incidents)]]" resource="[[resource]]"></rule-item>
                        <rule-edit features="[[features]]" hidden\$="[[!item.editing]]" active="{{item.editing}}" current-rule="[[item]]" resource="[[resource]]" resource-type="[[resourceType]]" model="[[model]]" available-metrics="[[availableMetrics]]" machines="[[machines]]" users="[[users]]" teams="[[teams]]"></rule-edit>
                    </template>
                </paper-material>
            </template>
            <paper-material elevation="0" id="add-new-rule-dialog">
                ...
                <rule-edit features="[[features]]" active="{{addingRule}}" available-metrics="[[availableMetrics]]" resource="[[resource]]" resource-type="[[resourceType]]" users="[[users]]" teams="[[teams]]" model="[[model]]" hidden\$="[[!addingRule]]">
                </rule-edit>
            </paper-material>
        </iron-collapse>
`,
