...
const BALANCE_GRAPH_OPTIONS = {
  tooltip: {
    ...
    formatter(params, _ticket, _callback) {
      const title = `${
        (params[0].data[1]
          ? `<strong>$${formatMoney(params[0].data[1], 2, '.', ',')} on `
          : ' ') + moment(params[0].data[0]).format('DD MMM')
      }</strong><br />`;
      const vcpus = params[0].data[2]
        ? `${formatMoney(params[0].data[2], 0, '.', ',')} vcpus<br />`
        : '';
      const datapoints = params[0].data[3]
        ? `${formatMoney(params[0].data[3], 0, '.', ',')} datapoints<br />`
        : '';
      const checks = params[0].data[4]
        ? `${formatMoney(params[0].data[4], 0, '.', ',')} rule checks<br />`
        : '';
...
Polymer({
  _template: html`
    ...
    <span class="due" hidden$="[[!amountDue]]"
      >Upcoming invoice for <strong>$[[_formatMoney(amountDue)]]</strong> will
      be billed at [[periodEnd]]</span
    >
    <div hidden$="[[!usageLoaded]]">
      <h2>Pay As You Go Usage</h2>
      <span class="burn" hidden$="[[!currentPaygRate]]"
        ><strong>Current PAYG rate:</strong> ~
        $[[_formatMoney(currentPaygRate)]]</span
      >
      <span class="burn" hidden$="[[!_isWithinPlan(currentPaygRate)]]"
        >Your current usage is within the limits of your plan.</span
      >
    </div>
    <div
      id="balanceGraph"
      style$="height:300px; width:[[balanceGraphWidth]]px;"
    >
  ...
  _formatMoney(i) {
    if (i) return formatMoney(i, 2, '.', ',');
    return i;
  },

  _isWithinPlan(paygRate) {
    if (paygRate === 0 && this.plan !== undefined) return true;
    return false;
  },
});
