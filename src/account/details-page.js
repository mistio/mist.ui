  ...
  _template: html`
        ...
        <paper-material>
            <div class="grid-row">
                ...
                <p class="xs12 l12">* Currently you cannot change your account's email for security reasons. Please contact us at <a class="blue-link" href\$="mailto:[[config.email.support]]">[[config.email.support]]</a>, if you wish to do so.</p>
                ...
                <div class="bottom-actions clearfix xs12">
                    ...
                    <iron-ajax id="userAjaxRequest" url="/api/v1/account" method="POST" loading="{{loadingUser}}" on-response="_handleAjaxResponse" on-error="_handleDetailsAjaxError" handle-as="xml"></iron-ajax>
                </div>
            </div>
        </paper-material>
        <paper-material class="separator">
            <div class="grid-row" hidden\$="[[!org.is_owner]]">
                <div class="xs12 head">
                    ...
                    <p>Enter your organization details. To switch organization use the  user menu in the top right corner of your screen.</p>
                </div>
                ...
                <paper-textarea class="xs12 l6" id="org-email" label="Organization Alert Email *" error-message="Please enter your email" value="{{orgAlertEmails}}" auto-validate="" hidden\$="[[!config.features.monitoring]]"></paper-textarea>

                <p class="xs12 l12" hidden\$="[[!config.features.monitoring]]">* In your organization's alert email, you can specify the email you want your incidents' alerts to be sent to. This can be overriden when you add rules in your monitored machines.</p>
                ...
                <div class="xs12 head">
                    ...
                    <div hidden\$="[[org.avatar.length]]">
                        ...
                        <div hidden\$="[[!customLogo]]">
                            <p class="xs12 l12">Upload an image that will override the default logo</p>
                            <vaadin-upload id="orgAvatarUpload" max-files="1" accept="image/png" max-file-size="262144" headers="[[_computeUploadHeaders()]]" target="/api/v1/avatars">
                ...
                <div class="xs12 margin-bottom" hidden\$="[[!config.features.r12ns]]">
                    <div hidden\$="[[!org.is_owner]]">
                        <paper-toggle-button checked="{{enableR12ns}}">
                            Recommendations <span hidden\$="[[enableR12ns]]">OFF</span> <span hidden\$="[[!enableR12ns]]">ON</span>
                        </paper-toggle-button>
                        <span class="secondary">Generate cost and performance recommendations for your monitored infrastructure.
                    </span></div>
                </div>
                ...
                <div class="xs12" hidden\$="[[!config.features.rbac]]">
                    <div hidden\$="[[!org.is_owner]]">
                        <paper-toggle-button checked="{{enableOwnership}}">
                            Private Resource Ownership <span hidden\$="[[enableOwnership]]">OFF</span> <span hidden\$="[[!enableOwnership]]">ON</span>
                        </paper-toggle-button>
                        <span class="secondary">Let users own the resources they create. Used for filtering and access control.
                    </span></div>
                </div>
                ...
                <div class="bottom-actions clearfix xs12">
                    <paper-button class="pull-right" disabled\$="[[!orgFormReady]]" raised="" on-tap="_submitOrgForm" page="second">Save Organization Profile</paper-button>
                    <iron-ajax id="orgAjaxRequest" url="/api/v1/org/[[org.id]]" method="PUT" loading="{{loadingOrg}}" on-response="_handleAjaxResponse" on-error="_handleOrgAjaxError" handle-as="xml"></iron-ajax>

                    <iron-ajax id="ownershipRequest" url="/api/v1/ownership" method="PUT" loading="{{loadingOrg}}" on-response="_handleAjaxResponse" on-error="_handleOrgAjaxError" handle-as="xml"></iron-ajax>
                </div>
            </div>
            ...
            <div class="grid-row" hidden\$="[[org.is_owner]]">
              <div class="xs12 head">
                  ...
                  <p>You are operating as a member of <strong>[[org.name]]</strong>. To switch between organizations use the user menu in the top right corner of your screen.</p>
              </div>
            </div>
...
`,
  ...
  properties: {
      ...
      orgFormReady: {
          type: Boolean,
          computed: '_computeOrgFormReady(orgName, orgAlertEmails, orgAvatar, enableR12ns, loadingOrg, org, enableOwnership)'
      },
      loadingUser: {
          type: Boolean,
          value: false
  ...
  _submitOrgForm(e, org) {
      ...
      if (this.orgName != org.name || this.enableR12ns != this.org.enable_r12ns || 
