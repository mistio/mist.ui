...
Polymer({
  _template: html`
                        ...
                        <template is="dom-repeat" items="[[notificationOverrides]]" as="override">
                            <paper-item class="row flex-horizontal-with-ratios">
                                ...
                                <div class="flexchild text-right">
                                    <paper-button class="red" tabindex\$="[[index]]" on-tap="_deleteOverrideTapped">Delete</paper-button>
                                </div>   
                            </paper-item>                        
                        </template>
                    </div>
        ...
        <iron-ajax auto="" id="getNotificationOverridesRequest" method="GET" url="/api/v1/notification-overrides" last-response="{{notificationOverrides}}" on-error="_handleGetOverridesError" handle-as="json"></iron-ajax>

        <iron-ajax id="deleteNotificationOverrideRequest" method="DELETE" on-response="_handleDeleteOverrideResponse" on-error="_handleDeleteOverrideError" handle-as="json"></iron-ajax>
`,
