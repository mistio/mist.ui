...
Polymer({
  _template: html`
    ...
    <paper-material>
      <div class="padding">
        ...
        <div
          id="datapointsGraph"
          style$="height:300px; width:[[graphWidth]]px;"
        >
          <paper-spinner active="[[loading]]"></paper-spinner>
        </div>
  ...
  resizeGraph() {
    ...
    if (this.coresGraph) this.coresGraph.resize();
    if (this.datapointsGraph) this.datapointsGraph.resize();
    if (this.checksGraph) this.checksGraph.resize();
  },
  ...
  _computeData(responseData, graph) {
    ...
    if (graph === 'cores') return timeToCores;
    if (graph === 'datapoints') return timeToDatapoints;
    if (graph === 'checks') return timeToChecks;
    return '';
  },
});
