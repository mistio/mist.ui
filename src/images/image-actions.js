  ...
  selectAction(e) {
    if (this.items.length) {
      ...
      if (action.name === 'unstar') {
        ...
        // single image only
        this.dispatchEvent(
          new CustomEvent('create_machine', {
            bubbles: true,
            composed: true,
            detail: { cloud: this.items[0].cloud.id, image: this.items[0].id },
          })
        );
      }
    }
  ...
  _star(action) {
    ...
    this.dispatchEvent(
      new CustomEvent('toast', {
        bubbles: true,
        composed: true,
        detail: { msg: `${action} request sent.`, duration: 1000 },
      })
    );
  },
  ...
  handleResponse() {
    this.dispatchEvent(
      new CustomEvent('toast', {
        ...
        composed: true,
        detail: {
          msg: `Successfully ${this.action.name}ed image.`,
          duration: 3000,
        },
      })
    );
