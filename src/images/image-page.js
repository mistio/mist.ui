  ...
  _template: html`
        ...
        <div id="content">
            <paper-material class="single-head layout horizontal">
                ...
                <image-actions actions="{{actions}}" items="[[itemArray]]"></image-actions>
            </paper-material>
            <paper-material>
                <div class="missing" hidden\$="[[!isMissing]]">Image not found.</div>
                <h4 class="id">Image ID:</h4><span class="id">[[image.id]]</span>
                <h4 class="id" hidden\$="[[!image.owned_by.length]]">Owner:</h4><span class="id"><a href\$="/members/[[image.owned_by]]">[[_displayUser(image.owned_by,model.members)]]</a></span>
                <h4 class="id" hidden\$="[[!image.created_by.length]]">Created by:</h4><span class="id"><a href\$="/members/[[image.created_by]]">[[_displayUser(image.created_by,model.members)]]</a></span>
                <h4 class="id tags" hidden\$="[[_isEmpty(image.tags)]]">Tags:
                </h4>
            ...
            <paper-material class="no-pad">
                <template is="dom-if" if="[[image]]" restamp="">
                    <mist-list id="imageLogs" frozen="[[_getFrozenLogColumn()]]" visible="[[_getVisibleColumns()]]" renderers="[[_getRenderers(model.members)]]" auto-hide="" timeseries="" expands="" column-menu="" searchable="" streaming="" infinite="" toolbar="" rest="" apiurl="/api/v1/logs" name="image logs" primary-field-name="time" base-filter="[[image.id]]"></mist-list>
                </template>
            </paper-material>
            <div class="absolute-bottom-right" hidden\$="[[isMissing]]">
                <paper-fab icon="star-border" on-tap="_starImage" id="starImageFab" hidden\$="[[!image.starred]]"></paper-fab>
                <paper-fab icon="star" on-tap="_starImage" id="starImageFab" hidden\$="[[image.starred]]"></paper-fab>
            </div>
        </div>
