...
Polymer({
    ...
    <paper-dialog id="dialogModal" with-backdrop="">
      ...
      <paper-dialog-scrollable>
        ...
        <template is="dom-if" if="[[!members.length]]" restamp="">
          <p class="margin">
            No users found.
            <a href="/teams" class="blue-link" dialog-dismiss=""
              >Add users in teams.
            </a>
          </p>
        </template>
            ...
            <div slot="dropdown-content" class="dropdown-content">
              ...
              >
                <template
                  id="membersDomRepeat"
                  is="dom-repeat"
                  items="[[_filteredMembers(members, membersFilter)]]"
                  as="member"
                >
                  <paper-item value="[[member.id]]"
                    >[[_displayUser(member)]]</paper-item
                  >
                </template>
              </paper-listbox>
            </div>
      ...
      <div class="clearfix btn-group">
        ...
        <paper-button
          class="red"
          disabled$="[[!newOwner]]"
          on-tap="confirmTransfer"
          >Transfer Ownership</paper-button
        >
      </div>
    </paper-dialog>
  `,
  ...
  confirmTransfer(e) {
    ...
    e.stopPropagation();
    this.dispatchEvent(
      new CustomEvent('transfer-ownership', {
        bubbles: true,
        composed: true,
        detail: { user_id: this.newOwner },
      })
    );
