...
MACHINE_CREATE_FIELDS.push({
  ...
  fields: [
    ...
    },
    {
      ...
      pattern: '[0-9]*',
      helptext:
        "The VM's size will be the size of the image plus the number in GBs provided here",
      helpHref: 'http://docs.mist.io/article/99-managing-kvm-with-mist-io',
    },
  ],
});
...
MACHINE_CREATE_FIELDS.forEach(p => {
  ...
  if (
    ...
    p.fields.push({
      ...
      required: false,
      helptext:
        'Start your Cloud Init script with #!/bin/bash or use a valid yaml configuration file starting with #cloud-config',
    });
  }
  ...
  // coming soon for 'gce', 'digitalocean', 'aws' & 'equinix metal'
  ...
  if (
      ...
      },
      {
        ...
        options: [
          ...
          },
          {
            ...
            type: 'mist_dropdown',
            helptext:
              "The machine's location must first be selected, to add existing volumes. Only volumes of the same location can be attached to a machine.",
            value: '',
            defaultValue: '',
            show: true,
            required: false,
            options: [],
            noOptionsMessage:
              'You must first select a location for your machine.',
            showIf: {
    ...
    if (['lxd'].indexOf(p.provider) > -1) {
      p.fields[p.fields.length - 1].options.push({
        ...
        options: [],
        helptext:
          'Path in the container the volume is attached. e.g. /opt/my/data. This is required when attaching the volume to a container',
      });
    }
    ...
    if (['ec2'].indexOf(p.provider) > -1) {
      p.fields[p.fields.length - 1].options.push({
        ...
        type: 'text',
        helptext:
          'Choose a device name. Recommended names /dev/sd[f-p] and /dev/sd[f-p][1-6]',
    ...
    if (['openstack', 'ec2', 'aliyun_ecs', 'vexxhost'].indexOf(p.provider) > -1) {
      p.fields[p.fields.length - 1].options.push({
            ...
            options: [
              {
                ...
                required: true,
                helptext:
                  'The public port, fill in either a port or host:port,\n eg: 80 or 172.11.234.13:80',
              },
              {
                name: 'target_port',
                label: 'Target Port',
                ...
                helptext:
                  'Optional, fill in only if the local port is different from the public one.',
              },
              {
                name: 'protocol',
                label: 'Protocol',
  ...
  p.fields.push(
    {
      ...
      defaultToggleValue: false,
      helptext:
        'Set an expiration date when the machine will stop or be destroyed',
      show: true,
      required: false,
      optional: true,
    ...
    },
    {
      ...
      required: false,
      helptext:
        'Example: */10 * * 1 *, is every 10 minutes on the 1st of each month. Relative periods: Minute, Hour, Day of the Month, Month of the Year, Day of the Week.',
      helpHref:
    ...
    },
    {
      ...
      defaultToggleValue: false,
      helptext:
        'Create an A record on an existing DNS zone that will point to the public ip address of the machine.',
      show: true,
      required: false,
      optional: true,
      singleColumnForm: true,
