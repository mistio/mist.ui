...
export const machinesListBehavior = {
    ...
    return {
      ...
      state: {
        body: (item, row) => {
          ...
          else ret += `<div class='state'>${item}</span>`;
      ...
      cloud: {
        ...
        cmp: (row1, row2) => {
          ...
          if (
            _this.model &&
            _this.model.clouds &&
            _this.model.clouds[item1.id] &&
            _this.model.clouds[item2.id]
          ) {
            if (
              _this.model.clouds[item1.id].title <
              _this.model.clouds[item2.id].title
            )
              return -1;
            if (
              _this.model.clouds[item1.id].title >
      ...
      cost: {
        body: (item, _row) => {
          ...
          return item && item.monthly
            ? _this.currency.sign + formatMoney(cost, 0)
            : '';
        },
        cmp: (row1, row2) => {
  ...
  _computeImage(row, item) {
    ...
    if (
      ...
      this.model.clouds[row.cloud.id].images &&
      this.model.clouds[row.cloud.id].images[imageId]
    ) {
