...
Polymer({
  _template: html`
    <style include="shared-styles">
      ...
      .details {
        ...
        transition: opacity 100ms ease-in, height 100ms ease-in,
      ...
      .details.open {
        ...
        transition: opacity 100ms ease-in, height 200ms ease-in,
          transform 200ms ease-in;
        overflow-y: scroll;
      }
      #paygDetails {
    ...
    <div id="widget" class="layout horizontal">
      <div id="viewMethod" class="view">
        Subscription type
        <div
          class$="plan-item [[_computeIsSelected('payg',selected)]]"
          on-tap="_selectPayg"
        >
          ...
          <div id="paygDetails" class="details">
            Free for up to 10 cpu cores, 100 datapoints/sec, 1000 rule
            checks/day, 10k API requests/day, 1 month retention
            <br />
            <strong>Over limits:</strong>
            <br />
            + $2 per cpu core/mo, + $5 per 1k datapoints/sec, + $5 per 100k rule
            checks/day, + $5 per 1M API requests/day
          </div>
        </div>
        <div class$="plan-item [[prepaidIsSelected]]" on-tap="_togglePrepaid">
          <span class="check"><iron-icon icon="icons:check"></iron-icon></span>
          Prepaid
          <span hidden$="[[!prepaidIsSelected]]"
            >[[selected]] [[_computePrice(selected)]]</span
          >
      ...
      <div id="viewPrepaid" class="view slideright">
        <paper-icon-button
          ...
          on-tap="_togglePrepaid"
        ></paper-icon-button>
        Prepaid Plans:
        <div class="plans-list layout horizontal">
          <div
            class$="plan-item [[_computeIsSelected('Small',selected)]]"
            on-tap="_selectSmall"
          >
            <div class="plan-title">
              <strong>Small</strong><br /><sub>$200/mo</sub>
            </div>
            <div class="details open">
              200 cpu cores<br />
              2k datapts/sec<br />
              100k rule checks/day<br />
              3 month retention
            </div>
          </div>
          <div
            class$="plan-item [[_computeIsSelected('Medium',selected)]]"
            on-tap="_selectMedium"
          >
            <div class="plan-title">
              <strong>Medium</strong><br /><sub>$700/mo</sub>
            </div>
            <div class="details open">
              1k cpu cores<br />
              10k datapts/sec<br />
              1M rule checks/day<br />
              1 year retention
            </div>
          </div>
          <div
            class$="plan-item [[_computeIsSelected('Large',selected)]]"
            on-tap="_selectLarge"
          >
            <div class="plan-title">
              <strong>Large</strong><br /><sub>$3,000/mo</sub>
            </div>
            <div class="details open">
              10k cpu cores<br />
              100k datapts/sec<br />
              10M rule checks/day<br />
              3 year retention
            </div>
          </div>
        </div>
  ...
  _computePrepaidIsSelected(selected) {
    return ['Small', 'Medium', 'Large'].indexOf(selected) > -1
      ? 'selected'
      : '';
  },
