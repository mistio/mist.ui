// import '@polymer/iron-timeago/iron-timeago.js';
...
Polymer({
        ...
        <div class="block">
          <template is="dom-repeat" items="[[enhancedIncidents]]">
            <div class="list layout horizontal wrap">
              <a
                class$="list-item flexchild layout horizontal [[_incidentColor(item,model)]]"
                href$="[[_computeResourceUri(item, model)]]"
                hidden$="[[!_computeResourceUri(item, model)]]"
              >
                <div class="flexchild expression">
                  <iron-icon
                    icon="[[_computeIncidentIcon(item, model)]]"
                    hidden$="[[!_computeIncidentIcon(item,model)]]"
                  ></iron-icon>
                  <span class="resource flexchild">
                    <span class="resource-name"
                      >[[_computeIncidentTitle(item, model)]]</span
                    ><br />
                    <span class="rule-condition"
                      >[[_computeIncidentCondition(item)]]</span
                    >
                  </span>
                  <span class="time">
                    <template is="dom-if" if="[[!item.finished_at]]">
                      <iron-timeago
                        datetime="[[_computeIncidentDuration(item)]]"
                      ></iron-timeago>
                    </template>
                  </span>
                </div>
          ...
          <a
            class="list-item layout horizontal wrap red"
            href="/incidents"
            hidden$="{{!resolvedCount}}"
          >
            <div class="flexchild expression">
              <iron-icon icon="icons:report-problem"></iron-icon>
              <span class="flexchild resource"
                >and {{resolvedCount}} more incidents in the last day</span
              >
            </div>
  ...
  observers: ['_openIncidentsChanged(openIncidents.*)'],
  ...
  _getResources(incidents) {
    if (incidents)
      return this.openIncidents.map(inc => {
        inc.resource = this._getResource(inc);
        return inc;
      });
    return [];
  },
    ...
    if (!item.logs[0].rule_arbitrary) {
      if (item.resource && item.resource.id)
        return `${item.resource.type} ${
          item.resource.name || item.resource.title || item.resource.domain
        }`;
      if (!item.resource || !item.resource.id)
  ...
  _computeIncidentDuration(item) {
    if (!item.finished_at) return new Date(item.started_at * 1000);
