...
Polymer({
  _template: html`
        ...
        <paper-input id\$="app-form-[[id]]-[[field.name]]-duration" placeholder="[[field.placeholder]]" label="[[field.label]]" validator="[[field.validator]]" value="{{span}}" auto-validate="" pattern="[[field.pattern]]" required="[[field.required]]" error-message="[[field.errorMessage]]" disabled="[[field.disabled]]" hidden\$="[[field.hidden]]" type="number" min="1" max="[[_computeMax(field.max,unit,span)]]" no-label-float="">
            <div suffix="" hidden\$="[[!field.suffix]]" slot="suffix">[[field.suffix]]</div>
            <div prefix="" hidden\$="[[!field.prefix]]" slot="prefix">[[field.prefix]]</div>
        </paper-input>
        <paper-dropdown-menu id\$="app-form-[[id]]-[[field.name]]" class\$="dropdown-block xs12 m6 [[field.class]]" auto-validate="" horizontal-align="left" disabled="[[field.disabled]]" hidden="[[field.hidden]]" required="[[field.required]]" no-label-float="">
            <paper-listbox slot="dropdown-content" id\$="app-form-[[id]]-[[field.name]]-duration-unit" attr-for-selected="value" selected="{{unit}}" class="dropdown-content">
