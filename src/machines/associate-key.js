...
Polymer({
  _template: html`
            ...
            >
              ...
              >
                <template is="dom-repeat" items="[[keys]]" as="key">
                  <paper-item
                    value="[[key.id]]"
                    disabled$="[[itemsHaveThisKey(key.id,items,model)]]"
                    >[[key.name]]</paper-item
                  >
                </template>
                <paper-button class="blue wide" on-tap="_addKey"
    ...
    <iron-ajax
      id="associateKeyRequest"
      method="PUT"
      on-response="_associateKeyResponse"
      on-error="_associateKeyError"
      on-request="_associateKeyRequest"
      handle-as="xml"
    ></iron-ajax>
  `,
  ...
  attached() {
    if (this.shadowRoot.querySelector('iron-dropdown')) {
      this.shadowRoot
        .querySelector('iron-dropdown')
        .setAttribute('vertical-offset', 55);
    }
  },
  ...
  _addKey(_e) {
    ...
    this.dispatchEvent(
      new CustomEvent('go-to', {
        bubbles: true,
        composed: true,
        detail: { url: '/keys/+add', params: qParams },
      })
    );
  },
  ...
  associateKey(e) {
    ...
    const run = (el, model) => {
      ...
      const machineHost = model.machines[itemId].public_ips
        ? model.machines[itemId].public_ips[0]
        : '';
  ...
  _associateKeyResponse() {
    ...
    this.dispatchEvent(
      new CustomEvent('toast', {
        ...
        composed: true,
        detail: {
          msg: 'Key association request sent successfully',
          duration: 3000,
        },
      })
    );
  ...
  _associateKeyError(e) {
    ...
    this.dispatchEvent(
      new CustomEvent('toast', {
        bubbles: true,
        composed: true,
        detail: { msg: e.detail.request.xhr.responseText, duration: 5000 },
      })
    );
  },
  ...
  computeModel() {
    return document.querySelector('app-main')
      ? document.querySelector('app-main').model
      : document.querySelector('mist-app').model;
  },
  ...
  itemsHaveThisKey(key, _items, _model) {
    ...
    for (let i = 0; i < this.items.length; i++) {
      ...
      for (let j = 0; j < model.keys[key].machines.length; j++) {
        if (
          model.keys[key].machines[j][0] === itemCloud &&
          model.keys[key].machines[j][1] === itemId
        ) {
          count++;
        }
      }
