...
Polymer({
  _template: html`
        <style include="shared-styles tags-and-labels info-table-style single-page">
            ...
            .machine-key iron-icon, .machine-volume iron-icon, .expiration iron-icon {
                margin-right: 8px;
                opacity: 0.32;
                cursor: pointer;
                width: 16px;
            ...
            .expiration iron-icon.delete, .machine-key:hover iron-icon.delete, .machine-volume:hover iron-icon.delete {
                opacity: 0.32;
            }
            ...
            @media screen and (max-width: 450px) {
                paper-menu-button[vertical-align="top"] ::slotted(.dropdown-content.paper-menu-button) {
        ...
        <div id="content">
            <machine-expiration-edit id="expirationdialog" machine="[[machine]]" model="[[model]]"></machine-expiration-edit>
            <paper-material class="single-head layout horizontal machine-page-head">
                ...
                <div class="title flex">
                    ...
                    <div class="subtitle">
                        ...
                        <span class="desc" hidden\$="[[_hasExtraStatus(machine)]]">[[_computeExtraStatus(machine, machine.*, model.machines.*)]]</span>
                    </div>
                </div>
                <machine-actions id="actions_machine" items="[[itemArray]]" model="[[model]]" in-single-page=""></machine-actions>
            </paper-material>
            ...
            <div class="columns">
                    ...
                    <div class="resource-info">
                        <div class="table">
                            <div class="row" hidden\$="[[!machine.cloud]]">
                                ...
                                <div class="cell">
                                    <iron-icon class="cloud icon" src\$="[[_computeCloudIcon(cloud.provider)]]"></iron-icon>
                            ...
                            <div class="row" hidden\$="[[!_getLocationName(machine,cloud)]]">
                                ...
                                <div class="cell">
                                    <span hidden\$="[[_canLinkToNetwork(machine.network)]]">[[machine.network]]</span>
                                    <span hidden\$="[[!_canLinkToNetwork(machine.network)]]">
                                        <a class="blue-link regular" href\$="/networks/[[machine.network]]">
                            ...
                            <div class="row" hidden\$="[[!machine.probe.ssh.updated_at]]">
                                ...
                                <div class="cell">
                                    ...
                                    <span class="smaller" hidden\$="[[!machine.probe.ssh.loadavg]]"> [[_listToString(machine.probe.ssh.loadavg)]] - [[machine.probe.ssh.cores]] vcpu cores</span>
                                </div>
                            </div>
                            <div class="row" hidden\$="[[!machine.probe.ping.updated_at]]">
                                ...
                                <div class="cell">
                                    <span class="smaller" hidden\$="[[!machine.probe.ping.rtt_avg]]">min/avg/max/std = [[machine.probe.ping.rtt_min]]/[[machine.probe.ping.rtt_avg]]/[[machine.probe.ping.rtt_max]]/[[machine.probe.ping.rtt_std]]
                                        ms
                                    </span>
                            ...
                            <div class="row" hidden\$="[[!_canShowProbed(machine)]]">
                                ...
                                <div class="cell">
                                    ...
                                    <paper-button id="probeButton" class="blue-link" on-tap="_probeMachine" disabled="{{probeLoading}}" hidden\$="[[!machine.key_associations.length]]">probe now</paper-button>
                ...
                <paper-material id="rightcolumn" class="right resource-description">
                    <div hidden\$="[[!machine.cost.monthly]]">
                        ...
                        <div class="cost">
                            [[currency.sign]][[_ratedCost(machine.cost.monthly, currency.rate)]]
                        </div>
                    </div>
                    <div class="row" hidden\$="[[!machine.owned_by.length]]">
                        ...
                        <div class="cell">
                            <a class="blue-link" href\$="/members/[[machine.owned_by]]">[[_displayUser(machine.owned_by,model.members)]]</a>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        ...
                        <div class="cell expiration" hidden\$="[[!machine]]">
                            ...
                            <template is="dom-if" if="[[canEditMachine]]" restamp="">
                                <iron-icon icon="editor:mode-edit" on-tap="editExpiration" class="edit"></iron-icon>
                            </template>
                            <span hidden\$="[[!machineExpiration]]">
                                <template is="dom-if" if="[[canDeleteExpiration]]" restamp="">
                                    <iron-icon icon="icons:clear" on-tap="deleteExpiration"></iron-icon>
                                </template>
                            </span>
                            <div class="machine-key" id="pendingExpRequest">
                                <paper-spinner active="[[isPendingExpirationRequest]]" hidden\$="[[!isPendingExpirationRequest]]"></paper-spinner>
                            </div>
                        </div>
                        <br>
                    </div>
                    <div hidden\$="[[!machine.key_associations.length]]">
                        ...
                        <div class="associatedKeys">
                            <template is="dom-repeat" items="{{machineKeys}}" as="association" id="machineKeysDomRepeat">
                                <div class="machine-key" hidden\$="[[!_visibleKey(model,association)]]">
                                    <a class="regular blue-link flexchild" href\$="/keys/[[association.key]]">
                                        <iron-icon icon="communication:vpn-key"></iron-icon> [[_computeKeyName(association.key, model.keys)]]</a>
                                    <iron-icon icon="icons:clear" on-tap="disassociateKey" class="delete"></iron-icon>
                                </div>
                            </template>
                    ...
                    <div hidden\$="[[!volumes.length]]">
                        ...
                        <div class="associatedVolumes">
                            ...
                            <div class="machine-volume" id="pendingVolumeRequest">
                                <paper-spinner active="" hidden\$="[[!isPendingVolumeRequest]]"></paper-spinner>
                            </div>
                        </div>
                        <br>
                    </div>
            ...
            <template is="dom-if" if="[[monitoring]]">
                <paper-material id="monitoringBlock" class="info-card">
                    <mist-monitoring id="mistMonitoring" resource="[[machine]]" state="[[machineState]]" monitoring="[[model.monitoring]]" section="[[section]]" datasources="[[datasources]]" hidden\$="[[hidden]]" machine-keys="[[machineKeys]]" incidents="[[_filterMachinesIncidents(model.incidentsArray, model.incidentsArray.length, model.incidents.*)]]" portal-name="[[portalName]]"></mist-monitoring>
                </paper-material>
            </template>
            <paper-material id="rulesBlock" class="info-card">
                <mist-rules id="monitoringRules" cloud="[[machine.cloud]]" builtin-metrics="[[model.monitoring.builtin_metrics]]" custom-metrics="[[model.monitoring.custom_metrics]]" incidents="[[model.incidentsArray]]" rules="[[_rulesApplyOnMachine(model.rules, machine.id, machineTags.*)]]" teams="[[model.teamsArray]]" users="[[model.membersArray]]" resource="[[machine]]" resource-type="machine" model="[[model]]" collapsible=""></mist-rules>
            </paper-material>
            ...
            <paper-material class="info-card" hidden\$="[[!_hasPowerInfo(machine)]]">
                ...
                <div class="card-content">
                    <element-for-in content="[[machine.extra.power_supply_info.Status]]"></element-for-in>
                    <element-for-in content="[[machine.extra.power_control_info]]"></element-for-in>
                    <element-for-in content="[[machine.extra.power_supply_info]]"></element-for-in>
                </div>
            </paper-material>
            <paper-material class="info-card" hidden\$="[[!_hasExtra(machine)]]">
                <h2 class="machine-page-head">More Info</h2>
                <div class="card-content">
                    <element-for-in content="[[machine.extra]]" ignore="power_"></element-for-in>
                </div>
            </paper-material>
            <paper-material class="machine-logs" hidden\$="[[!count]]">
                <template is="dom-if" if="[[machine.id]]" restamp="">
                    <mist-list id="machineLogs" timeseries="" expands="" column-menu="" searchable="" streaming="" infinite="" toolbar="" auto-hide="" rest="" apiurl="/api/v1/logs" name="machine logs" frozen="[[_getFrozenLogColumn()]]" visible="[[_getVisibleColumns()]]" renderers="[[_getRenderers(model.members)]]" primary-field-name="time" base-filter="machine_id:[[machine.id]]"></mist-list>
                </template>
            </paper-material>
        </div>
        <dialog-element id="machinedialog" btn-class="smaller"></dialog-element>
        <iron-ajax id="deleteExpirationDate" url="/api/v1/machines/[[machine.id]]" method="PUT" on-response="_handleMachineDeleteAjaxResponse" on-error="_handleMachineDeleteAjaxError"></iron-ajax>
        ...
        <iron-ajax id="disassociateKeyRequest" url="/api/v1/machines/[[machine.id]]/keys/[[disassociateKeyId]]" method="DELETE" on-response="_disassociateKeyResponse" on-error="_disassociateKeyError" on-request="_disassociateKeyRequest" handle-as="xml"></iron-ajax>
        <iron-ajax id="detachVolumeRequest" url="/api/v1/clouds/[[machine.cloud]]/volumes/[[dettachVolumeId]]" method="PUT" on-response="_detachVolumeResponse" loading="{{isPendingVolumeRequest}}" on-error="_detachVolumeError" handle-as="xml"></iron-ajax>
        <iron-ajax id="probeMachine" url="/api/v1/machines/[[machine.id]]/probe" method="POST" loading="{{probeLoading}}" on-error="_probeError" handle-as="xml"></iron-ajax>
...
`,
  ...
  properties: {
      ...
      machineSize: {
          type: String,
          computed: "_computeMachineSize(machine, machine.*, model.machines.*, cloud.*)"
      },
      machineExpiration: {
          type: String,
          computed: "_computeMachineExpiration(machine.expiration, machine.*, model.machines.*)"
      },
      machineTags: {
          type: Array,
