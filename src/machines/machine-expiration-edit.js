...
Polymer({
    ...
    <vaadin-dialog id="dialogModal" theme="mist-dialog" with-backdrop="">
      <template>
        ...
        <div class="paper-dialog-scrollable">
          <app-form
            id="machine-expiration-edit"
            fields="{{fields}}"
            single-column=""
            indialog=""
            no-horizontal-margins=""
            on-request="_expirationRequest"
            show-cancel=""
            btncontent="Save"
  ...
  _expirationRequest(_e) {
    this.dispatchEvent(
      new CustomEvent('pending-expiration-request', {
        bubbles: true,
        composed: true,
        detail: 'edit',
      })
    ...
    if (this.permissions === false) {
      this.dispatchEvent(
        new CustomEvent('toast', {
          ...
          composed: true,
          detail: {
            msg: 'You are not authorized to edit the expiration date.',
            duration: 3000,
          },
        })
      );
  ...
  _applyPermissions(machine, permissions) {
    ...
    if (currentExpiration || permissionsExpiration) {
      ...
      if (permissionsExpiration.actions) {
        action = permissionsExpiration.actions.default
          ? permissionsExpiration.actions.default
          : action;
      ...
      this.set(
        `${actionPath}value`,
        currentExpiration && currentExpiration.action
          ? currentExpiration.action
          : action
      );
  ...
  _initialiseValues(_machine) {
    ...
    const date =
      this.machine && this.machine.expiration
        ? this.machine.expiration.date
        : '';
    const fromNow = this._dateToDurationFromNow(date);
    const notify =
      this.machine && this.machine.expiration
        ? this.machine.expiration.notify
        : 0;

    this.set(
      'fields.0.subfields.0.value',
      this.machine.expiration
        ? this.machine.expiration.action
        : this.fields[0].subfields[0].defaultValue
    );
