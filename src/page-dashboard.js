...
Polymer({
        ...
        <div id="content" class$="flex-horizontal-wide onb-[[!showDashboard]]" main>
            <template is="dom-if" if="[[showDashboard]]">
                <div class="clouds top flexchild-wide-top">
                    <div id="cloudslist">
                        <template is="dom-repeat" items="[[_getFilteredResources(model.clouds,q)]]">
                            <a class="cloud-chip" href="/clouds/[[item.id]]">
                                <cloud-chip class$="link [[isOnline(item)]] [[isOffline(item)]]" index$=[[index]] id$="[[item.id]]">
                                    <div class="icon" slot="icon"></div>
                                    <div class="cloud-title" slot="cloud-title">[[item.title]]</div>
                                </cloud-chip>
                            </a>
                        </template>
                    </div>
                ...
                <div class="columns">
                        ...
                        <div class="sections layout horizontal wrap">
                            <template is="dom-repeat" items="{{sectionsArray}}">
                                <template is="dom-if" if="[[item.tile]]">
                                    <a href$="/[[item.id]]" on-tap="clearSearch" class="main-section flex" hidden$="[[_isHidden(item, item.count, sectionsArray)]]">
                                        <section-tile name$="[[item.id]]" color="[[item.color]]" icon="[[item.icon]]" count="[[_getSectionCount(item.id,sectionsArray,q)]]">
                                        </section-tile>
                                    </a>
                                </template>
                            </template>
                    ...
                    <div class="left">
                        <div class="costs">
                            <app-costs model="[[model]]" xsmallscreen="[[xsmallscreen]]" docs=[[docs]] currency=[[currency]] q="[[q]]">
                            </app-costs>
                        </div>
                        <template is="dom-if" if=[[model.org]] restamp>
                            <paper-material id="rightcolumn" class="logs">
                                <mist-list id="logs" timeseries expands searchable toolbar column-menu resizable streaming infinite rest apiurl="/api/v1/logs" name="all logs" frozen=[[_getFrozenLogColumn()]] visible=[[_getVisibleLogColumns()]] renderers=[[_getLogRenderers()]] base-filter="owner_id:[[model.org.id]]" primary-field-name="time" frozen-width="100"></mist-list>
                            </paper-material>
                        </template>
                    </div>
                </div>
    ...
    properties: {
        ...
        replaceTargets: {
            type: Object,
            computed: '_computeReplaceTargets(model.monitoring.machines.length, model.machines.*)'
        },
        openedCloud: {
            type: String
        },
