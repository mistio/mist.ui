...
Polymer({
    ...
    <div id="content">
      <paper-material class="single-head layout horizontal">
        ...
        <div class="title flex">
          <h2>[[script.name]]</h2>
          <div class="subtitle">[[script.location.type]] Script</div>
        </div>
        <script-actions
          items="[[itemArray]]"
          actions="{{actions}}"
          ...
          <div class="info-table">
            <div class="info-body info-group">
              ...
              <div
                ...
                hidden$="[[!script.owned_by.length]]"
              >
                <div class="flexchild">Owner</div>
                <div class="flexchild">
                  <a href$="/members/[[script.owned_by]]"
                    >[[_displayUser(script.owned_by,model.members)]]</a
                  >
                </div>
              </div>
              <div
                class="info-item flex-horizontal-with-ratios"
                hidden$="[[!script.created_by.length]]"
              >
                <div class="flexchild">Created by</div>
                <div class="flexchild">
                  <a href$="/members/[[script.created_by]]"
                    >[[_displayUser(script.created_by,model.members)]]</a
                  >
                </div>
              </div>
              <div class="info-item flex-horizontal-with-ratios">
              ...
              <template is="dom-if" if="[[!isInline]]">
                <div class="info-item flex-horizontal-with-ratios">
                  ...
                  <div class="flexchild">
                    <a
                      class="regular"
                      href$="[[_computeLink(script.location)]]"
                      target="_blank"
                    >
      ...
      <paper-material class="no-pad">
        <mist-rules
          ...
          rules="[[_rulesApplyOnResource(model.rules.*, script, script.tags.*, 'script')]]"
      ...
      <div class="absolute-bottom-right">
        <paper-fab
          ...
          id="runScriptFab"
        ></paper-fab>
        <paper-tooltip for="runScriptFab" position="left"
          >Run Script</paper-tooltip
        >
      </div>
    </div>
    <iron-ajax
      id="scriptDeleteAjaxRequest"
      url="/api/v1/scripts/[[script.id]]"
      method="DELETE"
      on-response="_handleScriptDeleteAjaxResponse"
      on-error="_handleScriptDeleteAjaxError"
    ></iron-ajax>
  ...
  _computeScriptValue() {
    return this.script
      ? this.script.location && this.script.location.source_code
      : null;
  },
