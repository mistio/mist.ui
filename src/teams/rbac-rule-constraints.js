...
Polymer({
  _template: html`
        ...
        <template is="dom-if" if="[[showConstraints]]">
            <dialog-element id="editConstraints" modal="true" formid="editConstraints-[[index]]" fields="{{fields}}" form="{{form}}" single-column-form="[[singleColumnForm]]" inline="[[inline]]"></dialog-element>
        </template>
`,
