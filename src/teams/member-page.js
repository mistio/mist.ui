...
Polymer({
  _template: html`
    ...
    <div id="content">
      <paper-material class="single-head layout horizontal">
        ...
        <div class="item-actions">
          <paper-menu-button no-animations="" horizontal-align="right">
            ...
            >
              <paper-button on-tap="_deleteMember" class="simple"
                >Delete Member</paper-button
              >
            </paper-listbox>
          </paper-menu-button>
        </div>
      ...
      <paper-material>
        <div class="flex-horizontal-with-ratios">
          <div class="page-block">
            ...
            <p>
              [[member.email]]
              <span class="pending" hidden$="[[!member.pending]]">Pending</span>
            </p>
          </div>
          <div class="page-block">
            ...
            <p>
              ...
              <template is="dom-repeat" items="[[memberTeams]]">
                <span class="team"
                  ><a class="regular blue-link" href$="/teams/[[item.id]]"
                    >[[item.name]]</a
                  ></span
                >
              </template>
      ...
      <paper-material class="no-pad">
        <mist-rules
          ...
          rules="[[_rulesApplyOnResource(model.rules, member, member.tags.*, 'member')]]"
    ...
    <iron-ajax
      id="deleteMember"
      url="/api/v1/org/[[model.org.id]]/teams/[[teamId]]/members/[[member.id]]"
      method="DELETE"
      on-response="_deleteMemberResponseAjax"
      on-error="_deleteMemberError"
    ></iron-ajax>
  `,

  is: 'member-page',

  behaviors: [mistLogsBehavior, mistRulesBehavior],
  ...
  _deleteMember(_e) {
    this._showDialog({
      ...
      body:
        'Deleting a member will also remove the member from all the teams they participate in',
      danger: true,
    ...
    if (this.memberTeams && this.memberTeams.length === 1) {
      this.dispatchEvent(
        new CustomEvent('go-to', {
          bubbles: true,
          composed: true,
          detail: { url: `/teams/${this.memberTeams[0].id}` },
        })
      );
    } else {
      this.dispatchEvent(
