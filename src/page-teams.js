          ...
          if (ownerRegex.exec(q).length) {
            ...
            if (owner) {
              if (owner === '$me') {
                if (
                  !item.members.length ||
                  item.members.indexOf(_this.model.user.id) === -1
                )
                  return false;
              } else {
                ...
                if (
                  !ownerObj ||
                  !item.members.length ||
                  item.members.indexOf(ownerObj.id) === -1
                )
                  return false;
              }
      ...
      name: {
        body: (item, row) => {
          if (
            ...
            _this.model &&
            _this.model.org &&
            _this.model.org.parent_org_name
          ) {
          ...
          if (
            ...
            const names = item.map(i => {
              if (_this.model.members[i]) {
                const displayName =
                  _this.model.members[i].name ||
                  _this.model.members[i].email ||
                  _this.model.members[i].username;
                return ` ${displayName}`;
              }
              return '';
            });
