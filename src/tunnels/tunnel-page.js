...
Script:</code></pre>
            ...
            <paper-material class="no-pad">
                <mist-rules id="tunnelRules" resource-type="tunnel" incidents="[[model.incidentsArray]]" rules="[[_rulesApplyOnResource(model.rules, tunnel, tunnel.tags.*, 'tunnel')]]" teams="[[model.teamsArray]]" users="[[model.membersArray]]" resource="[[tunnel]]" model="[[model]]" collapsible=""></mist-rules>
            </paper-material>
            <br>
            <paper-material class="no-pad">
                <template is="dom-if" if="[[tunnel]]" restamp="">
                    <mist-list id="tunnelLogs" frozen="[[_getFrozenLogColumn()]]" visible="[[_getVisibleColumns()]]" renderers="[[_getRenderers(model.members)]]" auto-hide="" timeseries="" expands="" column-menu="" searchable="" streaming="" infinite="" toolbar="" rest="" apiurl="/api/v1/logs" name="tunnel logs" primary-field-name="time" base-filter="[[tunnel.id]]"></mist-list>
                </template>
            </paper-material>
...
`,
  ...
  properties: {
      ...
      tunnelTags: {
          type: Array,
          computed: '_computeTunnelTags(tunnel, tunnel.tags, tunnel.tags.*, model.tunnels.*)'
      },
      isInline: {
          type: Boolean,
          value: true
