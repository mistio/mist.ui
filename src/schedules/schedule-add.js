      ...
      if (this.model && this.fields) {
          ...
          this.fields.forEach((f) => {
              ...
              if (f.name==="machines_uuids") {
                  f.options = this.model && this.model.machines && Object.keys(this.model.machines).map((m) => {
      ...
      if (changeRecord.path.endsWith('.value') && changeRecord.path.split('.value').length < 3) {
          ...
          if (this.get(changeRecord.path.replace('.value', '')).name.startsWith(
              ...
              if (!changeRecord.value.length || !parseInt(changeRecord.value, 10)) {
                  ...
                  this._updateObjectInSelectors('cost__monthly', 'value', parseInt(changeRecord.value, 10),
                      'gt')
              }
          }
          let entryInd;
          ...
          if (this.get(changeRecord.path.replace('.value', '')).name === "max_run_count") {
              ...
              if (typeof (this.get(`fields.${  maxcountInd  }.value`)) !== 'number') {
                  if (Number.isNan(parseInt(changeRecord.value, 10))) {
                      ...
                      this.set(`fields.${  maxcountInd  }.value`, parseInt(changeRecord.value, 10));
                  }
              }
          }
      }
