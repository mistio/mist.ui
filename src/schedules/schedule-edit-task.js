...
Polymer({
  _template: html`
                ...
                <div class="paper-dialog-scrollable">
                    ...
                    <div class="background" hidden\$="[[!_computeIsAction(taskType)]]">
                        <paper-dropdown-menu label="action">
                            <paper-listbox slot="dropdown-content" attr-for-selected="value" selected="{{newSchedule.action}}" class="dropdown-content">
                    ...
                    <div class="background" hidden\$="[[_computeIsAction(taskType)]]">
                        <paper-dropdown-menu label="script">
                            <paper-listbox slot="dropdown-content" attr-for-selected="value" selected="{{newSchedule.script_id}}" class="dropdown-content">
                        ...
                        <paper-textarea label="parameters" value="{{newSchedule.params}}"></paper-textarea>
                    </div>
                    ...
                    <div class="background" hidden\$="{{!_computeIsCrontab(newSchedule.schedule_type)}}">
                        ...
                        <span class="smaller">UTC Time only. Example */10 5 * * * . Space separated values for Minute, Hour, Day of the Month, Month of the Year, Day of the Week.</span>
                    </div>
                    <div class="background" hidden\$="{{!_computeIsOneOff(newSchedule.schedule_type)}}">
                        <div id="date" class\$="simple-button invalid-[[invalidDatetime]]">
                            ...
                            <paper-input id="datetime" type="datetime-local" value="{{datetime}}"></paper-input>
                        </div>
                    </div>
                    <div class="clearfix btn-group">
                        <paper-button tabindex="1" on-tap="_closeEditScheduleModal">Cancel</paper-button>
        ...
        <iron-ajax id="editSchedule" url="/api/v1/schedules/[[schedule.id]]" method="PATCH" loading="{{sendingData}}" on-response="_handleScheduleEditResponse" on-error="_handleScheduleEditError"></iron-ajax>
`,
  ...
  _computeNewSchedule(_schedule) {
      ...
      if (this.schedule){
          ...
          if (newSchedule.schedule_type === "interval"){
              ...
              this.set('datetime', moment.utc(this.schedule.schedule_entry.entry).local().format('YYYY-MM-DDTHH:mm'));
          }
  ...
  _updatePayload(_e) {
      ...
      if (this.schedule && this.newSchedule) {
          ...
          else if (this.taskType === 'script' && (this.schedule.task_type.script_id !== this.newSchedule.script_id || this.schedule.task_type.params !== this.newSchedule.params)) {
          ...
          if (this.schedule.schedule_entry && this.newSchedule.schedule_entry && this.newSchedule.schedule_type && JSON.stringify(this.schedule.schedule_entry) !== JSON.stringify(this._computePayloadEntry(this.newSchedule.schedule_entry)))  {
              pl.schedule_entry = this._computePayloadEntry(this.newSchedule.schedule_entry, this.newSchedule.schedule_type, this.datetime);
          }
          this.set('payload', pl);
      }
  ...
  _updateformReady(_sendingData) {
      ...
      if (!this.sendingData && JSON.stringify(this.payload) !== '{}') {
          ...
          if (((this.newSchedule.schedule_type === 'interval' || this.payload.schedule_type === 'interval') && this.payload.schedule_entry && !this._isValidInterval(this.payload.schedule_entry)) || ((this.newSchedule.schedule_type === 'crontab' || this.payload.schedule_type === 'crontab') && this.payload.schedule_entry && !this._isValidCrontab(this.payload.schedule_entry))) {
  ...
  _crontabEntryChanged(crontab) {
      if (this.schedule && this._computeIsCrontab(this.newSchedule.schedule_type) && crontab.value){
          this.set('newSchedule.schedule_entry', this._processCrontab(crontab.value));
      }
  }
});
