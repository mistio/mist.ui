                ...
                <paper-material hidden\$="[[!hasRecords]]">
                    ...
                    <template is="dom-if" if="[[renderRecordsList]]" restamp="">
                        <record-actions id="actions" items="[[selectedRecordItems]]" actions="{{recordActions}}" cloud="[[zone.cloud]]">
                            <mist-list id="recordsList" selectable="" column-menu="" multi-sort="" resizable="" items="[[zoneRecords]]" name="Records" selected-items="{{selectedRecordItems}}" frozen="[[_getFrozenLogColumn()]]" visible="[[_getVisibleColumns()]]" renderers="[[_getRenderers()]]" actions="[[recordActions]]">
                            </mist-list>
                        </record-actions>
                  </template>
                </paper-material>
