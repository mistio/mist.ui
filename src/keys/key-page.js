  ...
  _template: html`
        ...
        <div id="content">
            <paper-material class="single-head layout horizontal">
                ...
                <key-actions id="keyActions" actions="{{actions}}" items="[[itemArray]]" user="[[model.user.id]]" members="[[model.membersArray]]" org="[[model.org]]" in-single-view=""></key-actions>
            </paper-material>
            ...
            <div class="key-info">
                <paper-material>
                    ...
                    <h4 class="id" hidden\$="[[!key.owned_by.length]]">Owner:</h4><span class="id">[[_displayUser(key.owned_by,model.members)]]</span>
                ...
                <paper-material class="machines-list" hidden\$="[[!keyHasMachines]]">
                    <h4 class="key"><span>([[keyMachines.length]])</span> Machine<span hidden\$="[[!keyHasMachinesMoreThanOne]]">s</span> associated with this key</h4>
                    <ul>
                        <template is="dom-repeat" items="{{keyMachines}}">
                            <li>
                                <iron-icon icon="hardware:laptop"></iron-icon>
                                <span class="machine-info">
                                    <span hidden\$="[[!item.3]]">[[item.3]] @</span> <a hidden\$="[[!getMachineUrl(item,model.machines.*)]]" href="[[getMachineUrl(item,model.machines.*)]]" class="machine">[[getMachineName(item, model.machines.*)]]</a>
                                    <template is="dom-if" if="[[!getMachineName(item,model.machines.*)]]" restamp=""><abbr title\$="[[item.1]]">missing machine</abbr></template>
                                    <span>([[getCloudTitle(item)]])</span> <span hidden="[[!item.5]]">: [[item.5]]</span>
                                </span>
                            </li>
                        </template>
                    </ul>
                ...
                <paper-material class="no-pad">
                    <mist-rules id="keyRules" resource-type="key" incidents="[[model.incidentsArray]]" rules="[[_rulesApplyOnResource(model.rules, key, keyTags.length, 'key')]]" teams="[[model.teamsArray]]" users="[[model.membersArray]]" resource="[[key]]" model="[[model]]" collapsible=""></mist-rules>
                </paper-material>
                <br>
                <paper-material class="no-pad">
                    <template is="dom-if" if="[[key]]" restamp="">
                        <mist-list id="keyLogs" frozen="[[_getFrozenLogColumn()]]" visible="[[_getVisibleColumns()]]" renderers="[[_getRenderers(model.members)]]" auto-hide="" timeseries="" expands="" column-menu="" searchable="" streaming="" infinite="" toolbar="" rest="" apiurl="/api/v1/logs" name="key logs" primary-field-name="time" base-filter="[[key.id]]"></mist-list>
                    </template>
                </paper-material>
            </div>
        </div>
